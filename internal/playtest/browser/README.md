...
Native input accepts `hold`, `point` (absolute coordinate move), `click`, `wait`, and finite `gamepad` snapshots. A gamepad step exposes one `mapping: "standard"` virtual device through the page's ordinary `navigator.getGamepads()` API: four axes in `[-1,1]`, `lt`/`rt` button values in `[0,1]`, and the shared Xbox button names `a,b,x,y,lb,rb,ls,rs,start,back,guide,up,down,left,right`. It updates `connected` and `timestamp`, fills only a vacant Gamepad API slot without replacing a physical browser device, and returns to neutral after its bounded duration. This is browser-API injection for a private playtest lease, not native hardware emulation; a browser game must sample its normal Gamepad API as it would for a controller. Relative `move` remains unavailable because a page input API cannot provide native pointer-lock deltas.

A normal session/script completion calls `Cancel`, which leaves an idle browser intact. Cancellation while an input/RPC is active kills its whole child process group and its private browser context without waiting for a held gamepad step, then marks the lease unavailable. Release sends a neutral/disconnect state before context cleanup. The caller must stop or explicitly recover; no browser action is replayed or silently restarted.
